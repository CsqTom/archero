...
export class MonsterSkillCollider extends Component {
        ...
        if (otherCollider.getGroup() === constant.PHY_GROUP.OBSTACLE) {
            ...
            switch (this.colliderName) {
                ...
                case COLLIDER_NAME.LASER:
                    ...
                    this._hitMonster(otherCollider, scriptSkillCollider)
                    break;
                case COLLIDER_NAME.JET_FIRES:
                    scriptSkillCollider = this.node.parent?.getComponent(JetFires) as JetFires;
                    this._hitMonster(otherCollider, scriptSkillCollider)
                    break;
            }
        }
    ...
    private _hitMonster(otherCollider: any, scriptSkillCollider: any) {
        ...
        if (scriptMonster) {
            ...
            let scriptMonster = ndMonster.getComponent(Boss) as Boss;
            if (scriptMonster) {
                scriptMonster.reduceBlood(scriptSkillCollider.baseInfo);
            } else {
                // 没有玩家的小怪，只能打玩家了
